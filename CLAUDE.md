...
| 4 | 증식이 연령 연관의 얼마를 설명하는가(a: 공선성, **b: 증식 3분위별 연령 산점도**, c: 무작위 집합 효과 크기, **d: 전 유전자 18,135개의 연령 효과 대 센 속도 결합(ρ = −0.40, 사후)**; 코호트 정의·감쇠 분포·유지 유전자는 S2 d–f로) | `make_sa_fig3merged.R` |
| 5 | **거명 인자 여섯(HNRNPD·HNRNPA1·HNRNPK·SRSF1·SRSF2·SRSF3)을 유전자 하나하나로** — a: 세포를 센 자료의 무처리 HC1·HC2 57개를 배양 일수로, b: 공여자 107명을 10년 단위로(점 색 = 증식 3분위), c·d: GTEx 배양·일광 피부를 연령 구간으로; 맨 윗줄은 같은 시료의 증식 점수(§31) | `make_sa_fig5genes.R` |
| 6 | 63개 대비에서 결합이 일반화되는가(**b: 유전자 177 × 대비 63 heatmap**, 행은 센 속도 결합 순·열은 세포주기 변화 순; 옛 종류별 facet은 보충 S4a로) | `make_sa_fig3.R` |
| 7 | 어떤 노화 프로그램이 분열 읽기값인가 | `make_sa_fighall.R` |
| 8 | 결합은 조직에서도 성립하는가(사전등록 GTEx, **패널 a: 배양·하지 피부·치골상부 피부**(근육은 b의 heatmap에만), **패널 d: 구성 보정의 프로그램별 비교**) | `make_sa_figgtex.R` (Fig8.png·FigS8.png·보충 표 9 스코어카드를 함께 쓴다) |
...
| S4 | 종류별 결합(a, 옛 본문 Fig. 5b) + 개입(표본 외 예측, b–d) | `make_bio_fig6.R` |
...
**2026-09-18에 26개 전부에 하드 가드를 넣었다.** 각 파일 맨 위에 `FIGURES_WRITTEN` 선언과 `stop()`이 있어 그냥 실행하면 멈춘다. 일부러 돌려야 하면 `ALLOW_RETIRED_FIGURE_SCRIPT=1`을 준다. 가드를 지우지 않는다. 그림을 다시 만들 때는 위 표의 스크립트 16개만 쓴다.
...
## 27. 생물학적 단위와 그림 다듬기 (2026-09-20, 사용자 지적; 원고 v16)

**지적.** "거의 SD·중앙값·평균으로 scoring했는데 더 직관적인 생물학적 점수(P, FDR, logFC)를 포함하거나 대체하는 게 좋지 않나. 그리고 간격·글자 크기 때문에 가려지는 부분을 다듬어달라."

**단위.** `scripts/revision/run_biological_units.R` → `derived/biological_units/`. 집합 점수를 log₂ CPM으로 다시 계산해 배수 변화와 유전자별 FDR을 붙였다.

| | 값 |
|---|---|
| 센 속도, 중앙값 유전자 | 분열/일당 **+34%**, 0.1 분열/일당 +3.0% |
| 가장 빠른 10분위 대 가장 느린 10분위 | **1.30배(+30%)**, 177개 중 **172개**가 더 높다 |
| 센 속도에 대한 유전자별 FDR < 0.05 | **163개 증가, 3개 감소** |
| 공여자 연령, 중앙값 유전자 | 10년당 **−2.5%**, 76년 폭에 걸쳐 **−17%** |
| 증식 보정 후 | 10년당 **−0.7%** (= 기존 −0.040 SD) |

**함정**: 집합 점수는 유전자별 z의 평균이라 모든 유전자를 똑같이 가중하지만, log₂ CPM의 **평균**은 고발현 유전자가 지배한다. 그래서 두 요약이 대응하지 않는다(보정 전 환산계수 0.305 대 보정 후 0.244). **배수 변화는 반드시 "중앙값 유전자"로 쓴다** — 그래야 z 평균과 같은 가중이 된다. Methods에 "생물학적 단위" 절을 넣어 이 점을 밝혔다.

또 하나: 내 재계산은 보정 후 감소 유지 유전자를 18개로 주지만 원고의 19개는 코호트 스크립트의 파이프라인에서 나온 값이다. **모형이 다르면 새 숫자를 원고에 넣지 않는다** — 크기(%)만 추가하고 개수는 기존 값을 유지했다.

**보충 데이터 12 → 13.** Data 11에 유전자별 logFC·P·FDR 열 추가, Data 13은 공여자 코호트의 유전자별 연령 효과(보정 전후).

**그림.** Fig. 7d의 범례가 facet 제목 바로 위에 있어 "회색 = 기술 공변량"이 "sun-exposed skin"과 짝지어 읽혔다 → **범례를 축 아래로** 내렸다. Fig. 1·4·5·6·S3·S8을 눈으로 확인했고 그 밖의 가림은 없었다. **ggplot 범례를 facet 위에 두지 않는다** — 키와 strip이 세로로 겹쳐 잘못된 대응을 만든다.

## 26. 사슬을 잇는 세 분석 (2026-09-20, 사용자 요청 "①③④ 한 번에"; 원고 v15)

wet-lab 없이 절 사이 연결을 강화하는 세 가지. 전부 사후 분석이고 `scripts/revision/run_chain_strengthening.R` → `derived/chain/`.

**① 척도 통일 — 20개 marker 점수를 분열/일로 환산.** 센 라이브러리에서 `rate ~ prolif`를 적합하면 **점수 1 SD = 0.146 분열/일**(R² = 0.39). 그런데 **이 환산을 그대로 쓰면 효과가 2배로 부풀려진다** — 회귀 희석 때문이다(점수에 오차가 있어 기울기가 0 쪽으로 눌린다). 둘 다 있는 자료에서 계수 기반 2.90 SD/분열·일 대 점수 기반 5.67 → **보정계수 0.51**을 곱해 다른 자료에 적용한다. 결과가 이 작업의 핵심 산물이다:

| 자료 | 0.1 분열/일당 splicing 점수 변화 |
|---|---|
| 세포를 센 자료(계수) | **0.29 SD** |
| 공여자 107명 | **0.31 SD** |
| GTEx 배양 | **0.26 SD** |
| GTEx 일광 노출 피부 | 0.09 SD |
| GTEx 비노출 피부 | 0.04 SD |

배양 세 수집이 서로 다른 증식 측정을 쓰는데도 0.26–0.31로 모인다. 이게 여섯 절을 꿰는 한 숫자다. **주의**: 점수–splicing 상관(0.84, 0.91, 0.74)은 둘 다 같은 라이브러리에서 나와 기술적 변이를 공유하므로 계수 기반 0.71보다 높게 나오는 것이 정상이다. 상관은 상한, 효과는 보정 후 읽는다.

**③ 매개 모형.** 연령 → 증식 → splicing. 총효과 −0.215 SD/10년, 간접 −0.150 [−0.208, −0.092], 직접 −0.065 [−0.106, −0.029], **매개 비율 70% [54%, 85%]**(공여자 2,000회 부트스트랩). 직접 경로는 0을 안 지나간다 — 즉 이 코호트에서 잔여는 통계적으로 존재한다. **그러나 매개변수는 센 속도와 0.62로 상관하는 대리 지표이고, 고전적 측정오차 아래 간접 경로는 신뢰도만큼 감쇠한다. 신뢰도 0.70 이하면 완전 매개가 된다.** 0.62² ≈ 0.39이므로 잔여는 측정오차로 전부 설명된다. "증식과 독립인 연령 효과는 보고하지 않는다"는 원고의 자제를 뒷받침하는 가장 강한 근거다.

**④ bulk 조직 혼합 모형.** 각 점수의 공여자 간 변이를 분열 세포 비율(f)을 따르는 부분과 아닌 부분으로 나누면 `ρ(P,S) = w × ρ(P,C)`가 된다. 증식 점수와 세포주기 프로그램은 둘 다 f만 읽으므로 그 상관 0.67이 "설계가 f를 얼마나 잘 보는가"를 고정한다. 관찰된 splicing 0.23 → **w = 0.34**(모의실험 0.35로 일치). 즉 bulk 피부 splicing 점수의 공여자 간 변이 중 **약 1/3만**(분산으로는 1/8) f를 따라가고, 구성 보정이 그마저 0.23 → 0.10으로 줄인다. **조직에서 결합은 희석된 게 아니라 공여자 사이에서 대체로 없다.**

**작도 함정 하나.** 처음 혼합 모형을 `S = (1−w)·1 + w·f·(…)`로 짰는데 상관은 아핀 변환에 불변이라 w가 결과에 전혀 영향을 주지 않았다(모든 w에서 0.65). 비-f 성분은 **독립한 변동**이어야 한다: `S = w·z_f + sqrt(1−w²)·N`.

**그림 변화 없음.** Fig. 2f에 환산율 한 줄만 추가했고, 세 분석은 본문 문장 + 보충 표 3(f)·5(f)·10(g)로 들어갔다.

## 25. 검증 보강과 전사체 수준 증거 (2026-09-20, 사용자 지적; 원고 v14)

**지적.** "여섯 절의 논리가 약하다. 분열 속도를 다른 plot으로도 확인하고, splicing factor의 sashimi나 전사체 분석도 있어야 하지 않나. 지금은 scoring figure가 너무 많아 강력한 근거인지 모르겠다."

맞는 지적이었다. 본문 그림이 거의 전부 *점수 대 점수*였고, 두 가지 반론(① 평균 z 점수는 소수 유전자가 끌 수 있다, ② 상관은 직선을 가정한다)에 그림으로 답한 적이 없었다. 새 다운로드 없이 로컬 자료만으로 둘 다 답했다.

**Fig. 2에 추가한 두 패널** (`scripts/revision/run_gene_level_verification.R` → `derived/gene_level/`).
- **유전자 수준(패널 c)**: 발현 20,425개 유전자를 각각 센 속도와 상관. 177개 중 **98%가 양**(배경 20,248개는 44%), **42%가 배경 95 백분위수 위**(기대 5%), Wilcoxon *P* = 6 × 10⁻⁷⁵. **가장 강한 10개를 빼도 집합 점수는 0.71 → 0.69**(20개 빼면 0.68, 50개 빼면 0.65). 발현 맞춘 무작위 집합 1,000개의 유전자별 중앙값은 0.03(95% −0.04~0.09)이고 177개는 0.45.
- **용량–반응(패널 b)**: 센 속도 10분위에서 중앙값이 −0.60 → +0.69로 아홉 구간에서 상승(분위 중앙값의 Spearman 0.99). **누적 population doubling·배양 일수·세포주를 고정한 편상관 0.40**(*P* = 3 × 10⁻¹⁵) — 복제 나이가 아니라 채취 시점의 속도가 문제라는 근거.
- 중복이던 "무처리 세포 대 배양 일수" 패널은 뺐다(Fig. 1c와 같은 세포주·같은 관계). 그래서 Fig. 2 패널은 a–f로 다시 붙었다(옛 b→d, d→e, e→f).
- **쓰지 않은 것**: "시간당 배가시간(pdt)으로도 같다"는 독립 검증이 아니다. 속도는 24/pdt이므로 Spearman이 부호만 바뀐 같은 값이다(−0.706). 이런 단조 변환을 검증으로 제시하지 않는다.

**Fig. 3 신설: 전사체(junction) 수준** (`run_machinery_transcript_level.R` → `derived/machinery_transcript/`). GSE113957의 recount3 junction 행렬과 이미 만들어 둔 PSI 행렬(3,401 event × 142명)을 썼다. 유전자 배정은 `psi_age_events.tsv`의 `gene` 열.
- **SRSF3**: exon 3(…36,598,983)과 exon 5(36,601,151…) 사이의 짧은 exon. 증식 하위 1/3에서 건너뛰는 junction 71 / 포함하는 junction 10, 상위 1/3에서 **177 / 11** → 포함률 **12.6% → 6.1%**(ρ = −0.60, *P* = 7 × 10⁻¹²). 같은 유전자가 양(abundance)과 isoform 양쪽에서 같은 방향으로 움직인다.
- 검정 가능한 event를 가진 기계 유전자 28개 중 **12개에서 39개 event가 FDR < 0.05**(거명 인자 넷: HNRNPA1, HNRNPA2B1, HNRNPK, SRSF3). 한 유전자짜리 주장이 아니다.
- **집합 수준은 음성이고 그대로 싣는다**: 기계 event의 27%가 FDR < 0.05로 나머지 유전자의 30%와 다르지 않고, |*t*| 중앙값 2.00 대 1.49(*P* = 0.20). 그래서 "splicing 일반"이 아니라 "이 유전자들이 하는 일"이라고 쓴다. 이 음성 결과가 §11.15(outcome 주장 금지)와도 맞는다.
- **sashimi 작도 규칙**: exon 모델은 GENCODE v41(`ir_annotation_v41/exons.bed`)이고, **다른 exon을 완전히 포함하는 exon은 읽고 지나가는 형태로 보아 뺀다**. 빼지 않으면 SRSF3의 26 bp 대체 exon이 긴 read-through 블록에 묻혀 보이지 않는다. 호의 굵기는 평균 junction count, 창은 그린 junction의 범위(3′ 꼬리까지 그리면 정보 영역이 눌린다).

**번호 이동.** 새 Fig. 3이 들어가면서 옛 3–7 → 4–8. 단일 패스 정규식으로 원고 2 + 보충 Results 2 + Note 1 + 커버레터 2 + README + 빌더를 한 번에 바꿨고(`scratchpad/renumber_fig3.py`), 그림 스크립트의 `save_fig` 파일명도 높은 번호부터 바꿨다(충돌 방지). **`Figs 1, 2, 4 and 5` 같은 복수형과 한글 `Fig. 1·2·4·5` 형태는 별도 정규식이 필요하다** — 단수형만 바꾸면 조용히 어긋난다.

**보충 데이터 10 → 12.** Data 11 = 유전자별 센 속도 상관 전체, Data 12 = event별 PSI와 증식·연령 연관. "ten"이 적힌 곳은 README·원고 2(데이터/코드 가용성)·커버레터 2·빌더 목록 여섯 군데다.
...
## 30. 스토리 재점검 — 논리·근거·비약 (2026-09-21, 사용자 요청; 원고 v18)

사용자: "말이 안 되는 부분, 매끄럽지 못한 부분, 선행연구가 아닌 추론으로만 배경을 제시한 부분, 중심논리에서 벗어나거나 비약적인 부분을 배제." 원고 전문(초록·서론·결과·고찰·방법·참고문헌)을 다시 읽고 세 범주로 고쳤다.

**A. 인용 없이 추론으로 쓴 배경 (서론·고찰).**
- "spliceosome 조립·capping·3′-end processing은 신생 전사량에 비례해 요구된다" — 출처 없는 추론. → MYC가 전사를 증폭하면 core snRNP 유전자가 함께 유도되고 spliceosome이 커져야 한다^10,11^ + 증식 세포의 전사 프로그램^9^으로 바꿈. capping·3′-end는 문장에서 뺐다.
- **"고령 공여자의 세포는 젊은 공여자의 세포보다 느리게 분열한다"** — 출처 없고 논쟁적이며, **우리 자료의 절반(GTEx 배양, ρ = +0.10)이 반대**. 삭제. "계대마다 느려진다"는 ^13^, "개입이 분열 속도를 바꾼다"는 ^6,19^로 인용.
- "기계의 양을 재는 문헌은 주로 배양세포에서 그렇게 하며 두 측정을 바꿔 쓴다" — ref 1·4는 혈액이므로 "주로"가 틀리고 "바꿔 쓴다"는 인용 없음. → "많은 부분은 배양 섬유아세포^3,4,6,7^, hallmark 제안은 두 근거에 함께 기댄다^5^". 초록의 "most of the evidence" → "much of".
- "가장 강한 근거" (preprint ^8^) → "가장 직접적인 근거".
- 고찰 "산물이 신생 전사와 DNA 복제에 비례해 소모되는 프로그램" → "전사와 DNA 복제에 묶인 프로그램". Venet 2011은 "무작위 signature보다 나을 것이 없다, 둘 다 증식을 보고하기 때문"까지만; "세포주기 유전자를 빼도 의존이 안 사라진다"는 그 논문의 결과로 확인할 수 없어 삭제. senescence endpoint는 ^6,28^.

**B. 비약 (본문에서 내리고 보충에만).**
- **조직 혼합 모형(w = 0.34, "조직에서 결합은 희석이 아니라 대체로 없다")** — 증식 점수와 세포주기 프로그램이 같은 잠재변수만 읽는다는 가정 위의 상관 비율이라 본문 결론으로는 과하다. 본문은 "세포주기 0.67 대 splicing 0.23, 3분의 1 정도; 구성 보정 후 0.10"까지만 쓰고 모형은 보충 표 10에 둔다. §26 ④는 보충으로 격하.
- 매개 모형의 "그 상관(0.62)이 시사하는 신뢰도 0.70 이하" — 센 속도 자체가 대리 지표라 신뢰도를 그 상관으로 정할 수 없다. "0.70 이하이면 완전 매개; 0.62와 어긋나지 않지만 확정은 못 한다"로. "남긴 것이다" → "남길 만한 것이다".
- 고찰 "배양은 하나의 지배적인 변이 축을 부과한다" / 결과 6절·고찰 "분열 속도가 시료 사이의 주된 변수" — PC 분석 같은 근거가 원고에 없다. "크게 달라지고 결합된 프로그램을 끌고 간다(Fig. 6)"로.

**C. 흐름·모순.**
- 2절: "점수–splicing 상관 0.84가 더 높은 것은 같은 라이브러리의 기술 변이 공유 때문"이라고 단정했는데, 같은 절의 처리 문단은 "splicing 점수가 전사체 프로그램을 더 가깝게 따라간다(생물학)"고 말한다. 둘 다 부분 원인으로 병기.
- 2절 "선택 효과 둘, 어느 것도 결과를 설명하지 않는다" — 둘째(거명 패널이 계대로 정의됨)는 반박된 게 아니라 제거 불가. "첫째는 설명 못 함, 둘째는 없앨 수 없어 기술로만 보고".
- 3절 매개 문단 끝의 "점수가 계수를 덜 따라가므로 보정이 불완전…" 문장은 같은 문단의 반복 → 삭제. 나이 예측기 문장(r = 0.72…)은 중심 논리 밖 → 보충 Results 4 포인터 한 줄로.
- 5절 첫 문장 "Fig. 5는 Metabolism of RNA·translation을 이미 보였다" — Fig. 5에는 없다(본문 4절 수치) → "63개 대비는 … 보였다".
- 5절 methylation 문단은 사전등록 보고 의무를 지키되 시계별 CI 나열을 줄였다(네 결과 전부 보충 Fig. S7·표 8).
- 1절 "이전의 양성 결과 하나를 없앴다" — 독자가 볼 수 없는 옛 초고 언급 → "기탁 집합에서는 나타나고 이 코호트에서는 나타나지 않는 outcome 연관"으로.
- 고찰 자료 겹침 목록에 **Fig. 3(공여자 코호트의 junction)** 이 빠져 있었다 → 추가.

**규칙(추가).** 서론·고찰의 사실 문장마다 "이 문장의 출처는 무엇인가"를 묻는다. 출처가 우리 결과면 결과를 가리키고, 없으면 지운다. 우리 자료가 반대하는 통념(공여자 연령 → 분열 속도)을 배경으로 쓰지 않는다.

## 31. 노화 meta-analysis 형태의 그림 셋 (2026-09-21, 사용자 요청 "A, B, C 셋 다 추천대로"; 원고 v19)

**지적.** "통계적인 figure만 있다. scoring figure 말고 파고들 수 있는 figure, 다른 노화 meta-analysis 논문의 형태를 참고해라." 초안 세 개(A 유전자 × 대비 heatmap, B 거명 인자 여섯의 유전자별 곡선, C 전 유전자 volcano + 연령 효과 대 결합 산점도)를 만들어 보이고 셋 다 채택했다(volcano는 제외).

**들어간 것.**
- **새 Fig. 5** (`make_sa_fig5genes.R` → Fig5.png, 183 × 190 mm): *HNRNPD·HNRNPA1·HNRNPK·SRSF1·SRSF2·SRSF3* 를 행으로, 자료 넷을 열로. a 세포를 센 자료의 무처리 HC1·HC2(21% O₂, 57개) 배양 일수, b 공여자 107명 10년 단위(점 색 = 증식 3분위), c GTEx 배양 511명·d 일광 피부 739명의 연령 구간(GTEx gene sums를 `run_gtex_boundary.R`과 같이 filterByExpr·TMM·log-CPM으로 다시 정규화). 맨 윗줄은 같은 시료의 20개 marker 증식 점수. 수치(스크립트가 인쇄): 배양 일수 ρ −0.63 ~ −0.82; 공여자 10년당 −0.030 ~ −0.054 log₂ CPM, FDR 0.0003 ~ 0.035(보충 데이터 13), 증식 보정 후 유지 0/6; 80세 이상 33명 중 20명이 증식 하위 3분위; GTEx 연령 구간과의 ρ −0.08 ~ +0.15(여섯 유전자 모두 떨어지지 않는다). **GTEx 열은 "완전함을 위해" 그리고 근거로 쓰지 않는다고 본문·범례에 적었다**(사전등록 규칙 §10). 이 여섯은 집합 안 거명 인자 11개 중 SR 셋 + hnRNP 셋이고, 나머지 다섯은 Fig. 6b에 있다고 범례에 밝혔다.
- **Fig. 4d** (`make_sa_fig3merged.R`, 2 × 2, 183 × 150 mm): 두 자료 모두에서 측정된 18,135개 유전자의 연령 효과(공여자 코호트 limma) 대 센 속도와의 ρ. 전체 ρ = −0.40(*P* < 2 × 10⁻¹⁶), 177개 안에서는 −0.15(*P* = 0.048). 사후. **스크립트가 함께 인쇄하는 "177개 중 150개가 FDR < 0.05로 감소"는 원고에 넣지 않았다** — 원고의 137개는 코호트 파이프라인 모형의 값이고 이 DE 표는 다른 모형이다(§27의 규칙: 모형이 다르면 새 개수를 넣지 않는다).
- **Fig. 6b** (`make_sa_fig3.R` → Fig6.png, 183 × 198 mm): 177 × 63 heatmap. 셀 = 유전자 logFC − 그 대비의 전사체 평균(집합 점수와 같은 중심화; 스크립트가 `stopifnot`으로 저장된 대비 점수와 대조), ±1.5 클리핑, 결측 2.5%. 행은 `gene_vs_counted_rate.tsv`의 ρ 순, 열은 cc 순, 위에 종류색 막대(cc) + 점(집합 평균). 본문 수치: 유전자별 ρ(logFC, cc)의 중앙값 0.57, 177개 모두 양; 그 값과 센 속도 결합의 상관 0.51(*P* = 2 × 10⁻¹³). **63개 열에 accession을 전부 쓰면 읽히지 않는다** — 열 이름은 지우고 패널 a의 주요 실험 7개만 leader로 아래에 표시했다(인접 열 7·8은 fan-out 없이는 겹친다). 종류별 facet 여섯은 **보충 Fig. S4a**로 내려갔다(옛 a–c → b–d).

**번호 이동.** 옛 Fig. 5–7 → 6–8. 단일 패스(`scratchpad/abc_text_patch.py`): 원고 2 + 보충 Results 2(S4 패널 a→b, b→c) + Note 1(잠긴 본문의 "Fig. 8A"는 제외) + 빌더 + README + 템플릿 + 길잡이(판 이력 행은 제외) + 커버레터 2. 커버레터의 "Fig. 4b, d"는 옛 4d(코호트 정의)를 가리키던 묵은 인용이라 보충 Fig. S2d로 고쳤다 — **새 패널을 붙이면 같은 문자를 쓰던 묵은 인용이 살아나므로 grep 한다.** `make_sa_fighall.R` → Fig7.png, `make_sa_figgtex.R` → Fig8.png. 은퇴한 `make_sa_fig7summary.R`의 주석("Fig8.png는 더 이상 없다")은 Fig8.png가 GTEx 그림이 된 지금 틀리므로 고쳤다. 초안 PNG 세 장은 지우고 `scripts/drafts/`만 남겼다.

**작도 규칙(추가).** 여러 열의 facet 그림은 `lab_grid(..., align = "h", axis = "tb")`로 패널 상하를 맞춘다(범례 줄 수가 다르면 행이 어긋난다). 범례가 패널 폭을 넘으면 `guide_legend(nrow = 2, byrow = TRUE)`. heatmap의 회전된 y 제목은 패널 높이보다 짧아야 한다 — 두 줄로 끊고, 위 트랙의 제목과 겹치는지 본다.
